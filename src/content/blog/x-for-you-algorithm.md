...
推薦システムを初めて見ると、次のような処理を想像しやすいと思います。
...
小さなサービスなら、これでも動くかもしれません。しかし X 規模では、少なくとも 5 つの問題が起きます。

1 つ目は、**候補が多すぎる**ことです。世界中の投稿を、ユーザーが画面を開くたびに重い Transformer で採点することはできません。
...
2 つは競合ではなく補完関係です。複数経路を持つことで、1つのモデルが見落とした候補を別の経路が拾えます。
...
同時に、推薦システムを作る側にとっては、メーリングリスト モデルだけでは完成しないことがよく分かる実装です。候補生成、ルール、探索、多様性、障害耐性、実験基盤まで揃って、初めて 1 つのタイムラインになります。
