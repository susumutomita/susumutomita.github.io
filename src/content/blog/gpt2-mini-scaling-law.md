...
2020 年、OpenAI は "Scaling Laws for Neural Language Models" という論文で、LLM の性能がパラメータ数、データ量、計算量のべき乗則に従うことを示した。この発見は LLM 開発の方向性を決定づけ、GPT-3、GPT-4、そして Claude に至る大規模モデル競争の理論的根拠となった。
...
- L(N): パラメータ数 N のモデルの損失
- Nc: 臨界パラメータ数（定数）
- αN: パラメータスケーリング指数
- L∞: 理論的な損失の下限（不可約損失）

OpenAI の 2020 年論文では αN ≈ 0.076 と報告された。これは「パラメータ数を 10 倍にすると、損失が約 0.076 × log(10) ≈ 0.175 減少する」ことを意味する。

### 3 つのスケーリング軸

スケーリング則は 3 つの軸で成り立つ。

1. **パラメータスケーリング**: L(N) ∝ N^(-αN)
2. **データスケーリング**: L(D) ∝ D^(-αD)
3. **計算スケーリング**: L(C) ∝ C^(-αC)

これらは独立ではなく、最適な学習には三者のバランスが重要となる。2022 年の Chinchilla 論文はこのバランスの重要性を定量化し、「パラメータ数とデータ量は等しい割合で増やすべき」という知見を示した。

### なぜべき乗則が成り立つのか

べき乗則の理論的根拠は完全には解明されていないが、いくつかの仮説がある。

1. **統計力学的解釈**: ニューラルネットワークの損失関数のランドスケープが、スピングラスと類似の構造を持つ。
2. **情報理論的解釈**: 言語のエントロピー構造が階層的であり、より大きなモデルがより深い構造を捉える。
3. **近似理論**: より多くのパラメータがより複雑な関数を近似できる。
...
- Attention: 4 × n_embd²（Q, K, V, O の線形変換）
- MLP: 8 × n_embd²（fc: n_embd → 4×n_embd, proj: 4×n_embd → n_embd）
- LayerNorm: 2 × n_embd × 2（無視できる）
...
R² が 0.99 を超えており、べき乗則が非常によくフィットしていることを示す。
...
R² が高く、各点の誤差が小さいため、べき乗則の仮説は支持される。ただし、5 点では外挿の信頼性は限定的である。
...
ここで C は計算予算（FLOPs）である。この式は、計算予算が 10 倍になると、最適なパラメータ数は約 5.4 倍になることを意味する。
...
スケーリング則を用いると、目標性能を達成する最小コストを計算できる。

```
総コスト ∝ N × D ∝ L^(-1/α)
```

性能を 10％ 改善するのに必要なコスト増加率は約 30％ である（α = 0.076 の場合）。
...
1. **Emergent abilities**: 大規模モデルで突然現れる能力のスケーリング。
...
1. **べき乗則の観測に成功**: L = 20.89 × N^(-0.129)、R² = 0.994。
...
1. Kaplan, J., et al. (2020). [Scaling Laws for Neural Language Models](https://arxiv.org/abs/2001.08361). arXiv:2001.08361.
2. Hoffmann, J., et al. (2022). [Training Compute-Optimal Large Language Models](https://arxiv.org/abs/2203.15556). arXiv:2203.15556.
3. Brown, T., et al. (2020). [Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165). arXiv:2005.14165.
4. Wei, J., et al. (2022). [Emergent Abilities of Large Language Models](https://arxiv.org/abs/2206.07682). arXiv:2206.07682.
